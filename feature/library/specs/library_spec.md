...
This document defines the requirements, behavioral rules, and sync workflows for the library browsing, book metadata synchronization, and offline resilience within the `:feature:library` module and its supporting data layers.
...
- **`:domain`**: Use cases (`SyncLibraryBooksUseCase`, `GetBooksUseCase`, `GetPlaybackProgressUseCase`) exposing clean operations to ViewModels.
- **`:feature:library`**: Presentation layer containing `LibraryViewModel` and `LibraryScreen`.
...
2. **Reactive Updates**: Any local database mutation (from sync) automatically invalidates the Paging source and refreshes the UI without manual intervention.
